...
## What is the main contribution of this paper?
 ...
 This paper compares the performance of large language models on a set of challenging reasoning tasks from BIG-Bench using different prompting strategies. The main contributions are:
...
- BIG-Bench Hard (BBH): A subset of 23 challenging tasks from the BIG-Bench benchmark that prior language models could not exceed average human-rater performance on. The focus of analysis in this paper.

- Chain-of-thought (CoT) prompting: A prompting approach that includes intermediate reasoning steps and explanations in the few-shot examples. Enables improved performance over answer-only prompting. 

- Emergent capabilities: Abilities that emerge in larger model sizes but are not exhibited by smaller models. The paper shows how CoT unlocks emergent capabilities on several BBH tasks.

- Task performance: The paper evaluates few-shot performance via exact match accuracy between model outputs and ground truth labels. Compares answer-only vs CoT prompting.

- Model scaling: Analyzes how performance changes as model scale increases, focusing on OpenAI's InstructGPT and Codex as well as Anthropic's PaLM. 

- Algorithmic reasoning: Many BBH tasks require arithmetic, logical, spatial, etc reasoning. CoT prompts help decompose complex reasoning.

- Natural language understanding: BBH also includes semantic reasoning, sarcasm detection etc. where larger pretrained LMs already have some capabilities.

- World knowledge: BBH has tasks requiring real-world knowledge about sports, movies, causal relationships, etc. that pose challenges.

The key terms cover the benchmark, prompting methods, evaluation, and model analysis that are central to this paper's contributions.
...
3. What methodology did the authors use to test their hypothesis - for example, what type of study did they conduct and what data did they collect? 

4. What were the main findings or results of the study? Were the author's hypotheses supported or rejected?

5. Did the authors identify any limitations or shortcomings of their methodology or analysis? If so, what were they?

6. Do the findings confirm or contradict previous work in this research area? How do they build on the existing literature?

7. What conclusions or implications do the authors draw from their results? What is their assessment of the significance of the findings?

8. Do the authors suggest any real-world applications or next steps for future research based on their work? If so, what are they?

9. Does the paper present any novel concepts, frameworks, or models? If so, how do they work and what value do they add?

10. How robust and generalizable are the findings? Do the authors provide evidence to suggest the results could be reproduced or applied in other contexts?
