...
## Summarize the paper in one sentence.
 ...
 Unfortunately I cannot properly summarize the paper in one sentence as it seems to describe a method for training a neural network to predict correspondences between images, even for points that are occluded or outside the field of view. The key ideas seem to be using a non-siamesed architecture to allow linking information between images, a loss function called Neural Reprojection Error that can handle occluded/out-of-view points, and experiments showing the method can hallucinate plausible correspondences and improve camera localization compared to prior feature matching methods. Please let me know if you would like me to expand on any specific aspects of the paper in more detail.
