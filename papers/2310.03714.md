...
The paper proposes DSPy, a new programming model for designing AI systems using pipelines of pretrained language models (LMs) and other tools. DSPy allows developers to build pipelines by composing declarative modules with natural language typed signatures that abstract common LM operations like prompting, reasoning, and augmentation. DSPy modules are parameterized so they can learn to implement desired behaviors by bootstrapping useful demonstrations. The DSPy compiler optimizes arbitrary DSPy programs by simulating them to collect traces that are used to construct effective prompts or finetune smaller LMs. Optimization strategies are implemented via general teleprompters. Through case studies on math word problems and multi-hop question answering, the authors show DSPy programs using small LMs like Llama2-13b-chat and T5 can be quickly compiled to achieve performance competitive with or exceeding approaches relying on expert-written prompt engineering and large proprietary LMs. Overall, DSPy offers a programming model to develop high-quality LM pipelines from modular components instead of hand-crafted prompts.
